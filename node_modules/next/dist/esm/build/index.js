...
export default async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, experimentalBuildMode, traceUploadUrl) {
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            await recordFrameworkVersion("15.2.1-canary.0");
                ...
                await staticGenerationSpan.traceAsyncFn(async ()=>{
                    ...
                    staticPaths.forEach((prerenderedRoutes, originalAppPath)=>{
                        ...
                        const bypassFor = [
                            ...
                            ...isRoutePPREnabled && // Disable streaming metadata for PPR on deployment where we don't have the special env.
                        ...
                        for (const route of routes){
                            ...
                            const { revalidate = appConfig.revalidate ?? false, metadata = {}, hasEmptyPrelude, hasPostponed } = exportResult.byPath.get(route.pathname) ?? {};
                        ...
                        if (!hasRevalidateZero && isDynamicRoute(page)) {
                            ...
                            for (const route of dynamicRoutes){
                                var _route_fallbackRouteParams;
                                const normalizedRoute = normalizePagePath(route.pathname);
                                const { metadata, revalidate } = exportResult.byPath.get(route.pathname) ?? {};
                                let dataRoute = null;
                                if (!isAppRouteHandler) {
                                    dataRoute = path.posix.join(`${normalizedRoute}${RSC_SUFFIX}`);
                                }
                        ...
                        if (isSsg) {
                            ...
                            if (!isDynamic) {
                                ...
                                for (const route of additionalPaths.get(page) ?? []){
                                    ...
                                    const initialRevalidateSeconds = ((_exportResult_byPath_get3 = exportResult.byPath.get(route.pathname)) == null ? void 0 : _exportResult_byPath_get3.revalidate) ?? false;
