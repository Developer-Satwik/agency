...
import { encodeChildSegmentKey, encodeSegment, ROOT_SEGMENT_KEY } from '../../../server/app-render/segment-value-encoding';
 ...
 * entry, If not, it adds an empty entry to the cache and returns it.
 */ export function readOrCreateSegmentCacheEntry(now, // TODO: Don't need to pass the whole route. Just `staleAt`.
route, path) {
...
export function upsertSegmentEntry(now, segmentKeyPath, candidateEntry) {
...
export async function fetchRouteOnCacheMiss(entry, task) {
    ...
    try {
        ...
        const routeIsPPREnabled = response.headers.get(NEXT_DID_POSTPONE_HEADER) === '2';
...
export async function fetchSegmentOnCacheMiss(route, segmentCacheEntry, routeKey, segmentKeyPath) {
    // This function is allowed to use async/await because it contains the actual
    // fetch that gets issued on a cache miss. Notice it writes the result to the
    // cache entry directly, rather than return data that is then written by
    // the caller.
    ...
    try {
        const response = await fetchSegmentPrefetchResponse(href, segmentKeyPath === ROOT_SEGMENT_KEY ? // handling of these requests, we encode the root segment path as
        ...
        response.headers.get(NEXT_DID_POSTPONE_HEADER) !== '2' || !response.body) {
            // Server responded with an error, or with a miss. We should still cache
            // the response, but we can try again after 10 seconds.
    ...
    if (seedDataChildren !== null) {
        for(const parallelRouteKey in seedDataChildren){
            ...
            if (childSeedData !== null) {
                ...
                writeSeedDataIntoCache(now, route, staleAt, childSeedData, encodeChildSegmentKey(key, parallelRouteKey, encodeSegment(childSegment)), entriesOwnedByCurrentTask);
            }
        }
    }
}
